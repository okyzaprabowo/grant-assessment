...
class Questions_FormProcess{
    ...
    private function survey_form( $form_id ) {
        ...
        $html = '<form class="form" name="questions" id="questions" action="' . $_SERVER[ 'REQUEST_URI' ] . '" method="POST">';
        ...
        $html .='<br><br><br><br><br><table style="width: 95%;"><tr><td class="step-button" align="right">';
        if ( $actual_step == $next_step ):
            $html .= '<input class="questions_submission" type="submit" name="questions_submission" value="' . __(
                    'Go To Application', 'questions-locale'
                ) . '">';
        else:
            $html .= '<input class="questions_submission" type="submit" name="questions_submission" value="' . __(
	...
	public function text_thankyou_for_participation( $form_id ) {
		...
		$html .= 'setTimeout(\'Redirect()\',1000);';
		$html .= 'function Redirect(){location.href = "'.qu_change_email_return_name().'";}';
		$html .= '</script>';
		$html .= '<div id="questions-thank-participation" on>';
		$html .= '<p>' . __( 'Redirecting to application ...', 'questions-locale' ) . '</p>';
		if ( 'yes' == $show_results ) {
